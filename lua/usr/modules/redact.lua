...
M.patterns = {
  ...
  { pattern = '[Ee][Xx][Tt][Ee][Rr][Nn][Aa][Ll][Hh][Oo][Ss][Tt]:%s*[%w%._%-]+',
    replace = 'externalHost: REDACTED_EXTERNAL_HOST' },
  ...
  { pattern = 'spring%%.datasource%%.username[:=]%s*[%w._%-]+',
    replace = 'spring.datasource.username: REDACTED_DB_USER' },
  { pattern = 'database[Nn]ame=[^;,%s]+',                replace = 'databaseName=REDACTED_DB' },
}

-- Setup function: define commands and keymaps
...
-- Internal: apply all patterns on a line
...
function M.preview_buffer()
  ...
  -- Floating-preview keymaps:
  -- q → close window
  -- c → copy all preview lines to clipboard
